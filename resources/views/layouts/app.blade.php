...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('template/AdminLTE-3.0.5/plugins/icheck-bootstrap/icheck-bootstrap.css') }}">
    ...
    <link rel="stylesheet" href="{{ asset('dropzone-v5.5.1/dropzone.min.css') }}">
</head>
...
<body class="sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed control-sidebar-slide-open text-sm">
    ...
    <div class="wrapper">
        ...
        <aside class="main-sidebar elevation-4 sidebar-dark-danger">
            <!-- Brand Logo -->
            <a href="{{ route('home') }}" class="brand-link bg-red">
            ...
            <div class="sidebar bg-dark">
                ...
                <div class="user-panel mt-3 pb-3 mb-3 d-flex">
                    ...
                    <div class="info">
                        <a href="{{ route('users.config', Auth::user()->id) }}"
                            class="d-block text-white">{{ $nombre_usuario }}</a>
                    </div>
                </div>
                ...
                <nav class="mt-2">
                    ...
                    with font-awesome or any other icon font library -->
                        ...
                        @if (Auth::user()->tipo == 'CAJA')
                            @include('includes.menu.menu_caja')
                        @endif
                    </ul>
                </nav>
                <!-- /.sidebar-menu -->
        ...
        <input type="hidden" name="token" id="urlInactividad" value="{{ route('login.inactividad') }}">
        <!-- Main Footer -->
        <footer class="main-footer">
            <strong>Copyright &copy; {{ date('Y') }} Pocomas</strong>
    ...
    <script src="{{ asset('dropzone-v5.5.1/dropzone.min.js') }}"></script>
    <script>
        // Tiempo de inactividad deseado en milisegundos (ejemplo: 5 minutos)
        const tiempoInactividad = 1000; // 10 segundos

        let inactivoTimer;

        function reiniciarInactividad() {
            // Si hay un temporizador de inactividad existente, lo limpiamos
            if (inactivoTimer) {
                clearTimeout(inactivoTimer);
            }

            // Iniciamos un nuevo temporizador de inactividad
            inactivoTimer = setTimeout(function() {
                // actualizar sesión
                $.ajax({
                    headers: {
        ...
        function iniciarDetectorInactividad() {
            ...
            // Agregar los eventos que consideres relevantes para detectar actividad
            // Por ejemplo, si consideras que los clics son actividad, agrega:
            // document.addEventListener('click', actividadDetectada);

            // Si consideras que el movimiento del ratón es actividad, agrega:
            document.addEventListener('mousemove', actividadDetectada);

            // Si consideras que presionar una tecla es actividad, agrega:
            document.addEventListener('keypress', actividadDetectada);

            // Iniciamos el temporizador inicial
            reiniciarInactividad();
        }

        // Iniciar el detector de inactividad cuando se cargue la página
